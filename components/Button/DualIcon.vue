<template>
  <div class="flex flex-wrap gap-2 md:gap-4">
      <span v-for="(item,index) in itmes" :key="index">
          <div :class="`flex justify-around items-center space-x-2 px-4 py-2 text-sm rounded-lg font-semibold ${item.textColor} ${item.hoverTextColor} ${item.border} ${item.hoverlink} ${item.hoverBgColor}  ${item.bgColor}`">
