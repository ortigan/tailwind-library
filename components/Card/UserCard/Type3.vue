      ...
      <span v-for="(item,index) in items" :key="index" class="w-full h-full">
          <div :class="` overflow-hidden group w-full h-full relative rounded-xl ${item.shadowColor} ${item.hoverShadow} ${item.textColor} ${item.bgColor} ${item.border} ${item.hoverTextColor} ${item.hoverBgColor}`">
              <div class="h-96 w-full z-20">
                  <img class="h-full w-full object-cover group-hover:scale-110 transition-all duration-100 ease-in" src="https://res.cloudinary.com/pranav1421/image/upload/v1652079979/Tailwind-Component-library/Human%20avatar/payton-tuttle-RFFR1JjkJx8-unsplash_wglk5e.jpg" alt="">
              </div>
             <div class="h-full flex flex-col justify-end absolute top-0 w-full z-40">
                 <div class="-mb-40 group-hover:-translate-y-40 transition-all duration-200 ease-in">
                 <div class="w-full flex flex-col items-center justify-center p-4 bg-black/70">
                     <div :class="`text-lg ${item.textColor}`">{{item.title}}</div>
