<template>
  <div class="w-full">
    <span v-for="(item, index) in items" :key="index">
      <div
        :class="`bg-white flex justify-between items-center px-4 py-3 rounded-lg text-gray-500 lg:w-1/3 ${item.bgDivColor} mb-2`"
      >
        <div class="flex items-center space-x-2">
          ...
          <div :class="`${item.textColor}`">
            {{ item.text }}
          </div>
        </div>
        <div :class="`cursor-pointer ${item.bgCrossColor} rounded-lg p-1 ${item.iconcolor} ${item.hoverCrossColor} transition-all`">
          <svg
