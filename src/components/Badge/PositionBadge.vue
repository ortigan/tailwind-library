<template>
  <div class="flex flex-wrap items-center">
    <span v-for="(item, index) in itmes" :key="index">
      ...
      >
        {{ item.title }} <span :class="`text-xxs bg-red-500 rounded-full ml-1 ${item.normalCss}`">{{item.normalStat}}</span>
        <div :class="`absolute flex items-center text-xxs bg-red-500 rounded-full ${item.css}`">
            {{item.PositionedStat}}
        </div>
      </div>
    </span>
