<template>
  <div class="flex flex-wrap gap-4">
    <span v-for="(item, index) in itmes" :key="index">
        <div :class="`py-0.5 px-2 flex items-center space-x-1 justify-between text-xxs rounded-md font-normal hover:cursor-pointer ${item.textColor} ${item.hoverTextColor} ${item.border} ${item.hoverlink} ${item.hoverBgColor}  ${item.bgColor}`">
      <div type="button">
